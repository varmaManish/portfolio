...
// ── DATE HELPERS ────────────────────────────────────────────
// "MAY 2026" → "2026-05-01"  (for <input type="date">)
function parseDisplayDateToISO(str) {
  const map = { JAN:'01',FEB:'02',MAR:'03',APR:'04',MAY:'05',JUN:'06',
                JUL:'07',AUG:'08',SEP:'09',OCT:'10',NOV:'11',DEC:'12' };
  const parts = (str || '').toUpperCase().trim().split(/\s+/);
  if (parts.length === 2 && map[parts[0]] && /^\d{4}$/.test(parts[1])) {
    return `${parts[1]}-${map[parts[0]]}-01`;
  }
  return '';
}
// "2026-05-15" → "MAY 2026"  (stored in Firestore)
  ...
  document.getElementById('drawerBody').innerHTML = `
    ...
    <div class="field-row">
      <div class="field-group">
        ...
        <input class="field-input" id="b-date" type="date" value="${parseDisplayDateToISO(b?.date || '')}">
      </div>
      <div class="field-group">
...
document.getElementById('drawerSave').addEventListener('click', async () => {
  ...
  if (drawerMode === 'project') {
    ...
    await saveBlog({
      title,
      date:      formatISOToDisplayDate(document.getElementById('b-date').value),
...
// Wire up color pickers — update live preview on change
document.addEventListener('DOMContentLoaded', () => {
  Object.keys(DEFAULT_COLORS).forEach(varName => {
    const swatch = document.getElementById('clr' + varName);
    if (!swatch) return;
    swatch.addEventListener('input', () => updatePreview());
  });
});
